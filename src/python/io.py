class LammpsData:
    ...
    def write(self, filename):
        ...
        with open(filename, "w") as f:
            ...
            f.write(f"{self.angles.num_angles} angles\n")
            f.write(f"{self.atoms.num_types} atom types\n")
            f.write(f"{self.bonds.num_types} bond types\n")
            f.write(f"{self.angles.num_types} angle types\n\n")
    ...
    def polyWrap(self):
        ...
        このとき，結合が切れないように原子座標を調整する
        重心がシミュレーションセルに収まるように原子座標を調整する
        """
        for i in range(1, self.atoms.num_mols + 1):
            # 分子IDが i の原子のインデックスを取得, index は 1 から始まる
            idx = [j for j, mol_id in enumerate(self.atoms.mol_id) if mol_id == i]
            # 分子内の原子座標を取得
            coords = [self.atoms.coords[j] for j in idx]
            # 1. 分子内で「アンラップ」する
            # 最初の原子はそのまま採用
            ...
            # 2番目以降の原子について、前の原子との距離が
            # セルの半分を超える場合は周期境界をまたいでいると判断し補正する
            for k in range(1, len(idx)):
                ...
                for d, axis in enumerate(["x", "y", "z"]):
                    ...
                    shift_image = int(round(diff / L))
                    new_coord.append(current_coord[d] - shift_image * L)
                    new_image.append(shift_image)
                unwrapped_coords.append(new_coord)
                unwrapped_image_flag.append(self.atoms.image_flag[idx[k]] + tuple(new_image))

            # 3. 分子の重心を計算（各原子の質量が等しいと仮定）
            n_atoms = len(unwrapped_coords)
            com = [
                sum(coord[d] for coord in unwrapped_coords) / n_atoms for d in range(3)
            ]

            # 4. 重心がセル内に収まるように全体を平行移動するためのシフト量を算出
            shift = []
            shift_image = []
            for d, axis in enumerate(["x", "y", "z"]):
                lo, hi = box_bounds[axis]
                L = box_lengths[axis]
                # 重心が [lo, hi] に入るように（剰余演算を利用）
                new_com = (com[d] - lo) % L + lo
                shift.append(new_com - com[d])
                shift_image.append(int((com[d] - lo) / L))

            # 5. シフトを適用して最終的な座標とし，元の座標リストを更新する
            wrapped_coords = []
            new_image_flag = []
            ...
            for image_flag in unwrapped_image_flag:
                new_image = [image_flag[d] + shift_image[d] for d in range(3)]
                new_image_flag.append(new_image)
            for index, new_coord in zip(idx, wrapped_coords):
                self.atoms.coords[index] = tuple(new_coord)
            for index, new_image in zip(idx, new_image_flag):
                self.atoms.image_flag[index] = tuple(new_image)
